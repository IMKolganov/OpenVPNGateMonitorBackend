-- Update free-tier channel subscribe reminder email template (deep link)
UPDATE xgb_dashopnvpn."EmailBroadcastTemplates"
SET
  "Description" = $mightml$Built-in: admin email remind — subscribe to channel and/or link Google↔Telegram. Placeholders: {{DISPLAY_NAME}}, {{REQUIRED_CHANNEL}}, {{CHANNEL_URL}}, {{LINK_CODE}}, {{LINK_TTL_MINUTES}}, {{CODE_LABEL}}, {{CODE_VALUE}}, {{ACTION_URL}}, {{ACTION_LABEL}}; optional block <!--BEGIN_LINK_ACCOUNT-->…<!--END_LINK_ACCOUNT-->$mightml$,
  "Subject" = $mightml$Subscribe to our Telegram channel and link your account — DataGate$mightml$,
  "BodyHtml" = $mightml$<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8" />
...
-- verify
SELECT "Name", "Subject", POSITION('BEGIN_LINK_ACCOUNT' IN "BodyHtml") > 0 AS has_link_block,
       POSITION('{{ACTION_URL}}' IN "BodyHtml") > 0 AS has_action_url, "LastUpdate"
